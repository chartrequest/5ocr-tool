#!/bin/bash
#This tool must be run with sudo and it will help set up cr-tool dependencies
...
if ! command -v 5ocr_tool &>/dev/null; then
  cp -u $(dirname ${0})/cr-tool /usr/local/bin || exit 1
  chmod 755 /usr/local/bin/cr-tool
fi

#Generate a machine ID to avoid dbus errors
systemd-machine-id-setup
echo "All set! You may now run 'cr-tool login' to obtain your temporary keys"
